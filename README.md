...
Note の紐付けキーの決め方を変えたため、次のいずれかに当てはまる既存 Note は
...
- **symlink 経由で開いていたファイルの Note** — 実ファイル1つにキー1つとなるよう
  symlink を解決するようになりました

該当ファイルを開くと、未接続の sidecar がある旨を1度だけ通知します。
ただしプロジェクトルート自体を symlink 経由で開いていた場合の sidecar は、
そのプロジェクトを初めて開いたときに自動で統合されるため、取り込み直しは不要です。
...
入れ替わったときだけ「差し替わった」と判定します。行の比較は前後の空白を無視するので、
保存時の行末空白除去やインデントの一括変更では判定は変わりません。短い文書
...
- プロジェクトごと移動・改名した場合、sidecar 名のハッシュが変わるため既存 Note は
  ...
  （記録されたパスのファイルが現在のプロジェクトに実在する sidecar も候補になります）
