...
- 1割未満でも、**段落**サンプルの1割以上が残っている → `same`（折り返し幅の変更）
- どちらも1割未満 → `replaced`
- サンプルが10件未満 → `unknown`（比率が意味を持たない。短い文書は見出し1行を共有するだけで
  似てしまうので、告発も保証もしない。`:ContextMarkRelocate` が短いファイル同士を総当たりで
  移動先候補に出していたのはこれが原因）
- サンプルが無い（指紋未記録）、または現在のファイルが空行だけ → `unknown`。**`replaced` として扱わない**
  （空行だけのファイルは `anchor.resolve()` が `orphaned` を返す）

段落サンプル（`paragraphs`）は、空行で区切った非空行の並びを1つの文字列にしたものの digest。
折り返し直し（prettier `proseWrap`、`gq` など）は行を全部変えるが段落の中身は変えないので、
行サンプルだけだと全 Note が `mismatch` になっていた。60文字未満の段落（見出し・`---` など、
同じテンプレートの別文書が共有する定型）と4000文字超の段落（空行の無いファイル全体）は数えない。
記録済みの段落が4件未満なら使わない。この欄は追加専用で、欄の無い旧指紋は行だけで判定する。

`compare()` は3つ目の戻り値として、判定の根拠になったサンプルが残った割合を返す。通常は行サンプルの
割合だが、段落で救済したときは**段落サンプルの割合**になる。`render.lua` はこれが
**5割未満で `same`** のとき、Note 側にも聞く（`notes_recognize`）: どの Note も自分の前後文脈
（`before` / `after`）の行を見つけられなければ `replaced` として扱う。テンプレート由来の別文書
（ADR など）は定型行だけで1割を超えるため、ファイルだけでは同じと判定してしまうからだ。
**Note の excerpt 自体は数えない。** `## Decision` のような定型行に付いた Note は、同じテンプレートの
どの文書でも excerpt が見つかってしまう。逆に、大きく書き換えても Note の前後が残っていれば同じファイル。
**前後文脈を見るのは excerpt が見つかった（`exact` / `moved`）Note だけ。** `stale` の位置は元の行番号への
フォールバックで、テンプレートの別文書では同じ行番号の直前に同じ見出しがあるため、本文が消えていても
「前後が残っている」と誤認する。

段落で救済したときに Note に聞かないのは意図的。折り返し直しは Note の前後の行も書き換えるので、
聞けば自分のファイルの整形だけで全 Note が `mismatch` になる（段落救済を入れた理由そのもの）。

閉じているファイルは render が走らないので、`init.lua` の `comments_for()` が prompt を作る直前に
ファイルごとに1回ディスクから読んで `compare()` する（`flag_replaced_closed_files`）。`replaced` なら
その prompt に限り `mismatch` として出す。sidecar は書き換えない（ファイルが戻れば自然に消える）。
...
- 行は前後の空白を落とし、行内の連続する空白を1つに畳んで比較する（`canonical`）。保存時の行末空白除去や一括インデントは、
...
さらに `identity.lua` は行ごとの digest を生の行文字列をキーにメモ化する（`line_digest`）。
編集や `:w` でキャッシュが外れても、変わった行だけハッシュし直せば済む（5万行で約160ms → 約30ms）。
保存される digest の値は変えていないので、既存の指紋と互換。
...
区別し、読めないとき（JSON 壊れ・未知 version・存在するのに開けない）は `M.save` が理由付きで
失敗を返す。「無い」は `io.open` の失敗だけで判定せず `fs_stat` で実在しないことを確かめる
（権限エラーを「無い」と読むと、次の保存が rename で既存 sidecar を置き換える）。
...
3つ目は `save()` の排他ロック（`<sidecar>.lock` に LuaJIT ffi 経由で `flock(LOCK_EX | LOCK_NB)`）。
atomic rename が守るのは「読み手が半端なファイルを見ない」ことだけで、2つのインスタンスがそれぞれ
read → merge → write を走らせると後の rename が先の結果を捨てる。ロックが取れなければ
**黙って上書きせず失敗を返す**。

ロックは「ファイルがあるか」ではなく flock で持つ。存在ベースのロックはクラッシュ後に名前で消す
必要があり、「古いか確かめてから消す」はどう組んでも他インスタンスの生きたロックを消す隙間が残る
（それを直列化する2つ目のロックにも同じ隙間が移るだけ）。flock はプロセスが死ねばカーネルが外すので、
古さの判定も削除も要らない。**`.lock` ファイルは絶対に unlink しない** — flock 中のファイルを消すと、
片方が旧 inode、もう片方が新しいファイルをそれぞれロックできてしまう。flock が使えない環境
（Windows、LuaJIT 無しのビルド）ではロック無しで保存する。
...
ファイルのままであることを確認する（`render.judge ~= "replaced"`）。`identity.compare` を直接使うと、
テンプレート由来の別文書を定型行だけで同じファイルと判定し、Note をそちらへ運んでしまう。
...
- sidecar の JSON スキーマを変える場合は `store.lua` の `version = 1` と読み込み時の
  バージョンチェックを更新する（現在は version 不一致を `unreadable` として読み込みを拒否し、
  `save()` も上書きを拒否する。空 state として扱うと次の保存で全 Note が消えるため）。
