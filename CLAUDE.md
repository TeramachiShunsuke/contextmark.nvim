...
- 1割未満 → `replaced`
- サンプルが10件未満 → 1件も残っていないときだけ `replaced`（比率が意味を持たない）
...
日常編集の誤検知を直そうとすると逆にテストが邪魔をする。`tests/run.lua` の `document()` を使う。
...
`:ContextMarkAdopt`（`init.lua` の `M.adopt`）が取り込む。root の導出を変える変更は、
この移行経路と README の破壊的変更の記述を必ずセットで更新すること。
